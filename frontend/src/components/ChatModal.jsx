...
export default function ChatModal({ target, onClose }) {
  ...
  const send = async () => {
    ...
    const optimistic = { id:"tmp-"+Date.now(),sender_id:user.id,receiver_id:target.id,content:text,created_at:new Date().toISOString(),is_system:false };
  ...
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",display:"flex",alignItems:"flex-end",justifyContent:"flex-end",padding:"1rem",zIndex:1000 }}
      ...
      <div style={{ width:400,maxHeight:"75vh",background:"#fff",borderRadius:16,border:"0.5px solid #e2e8f0",display:"flex",flexDirection:"column",boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
        <div style={{ padding:"14px 16px",borderBottom:"0.5px solid #f1f5f9",display:"flex",gap:12,alignItems:"center" }}>
          <div style={{ width:38,height:38,borderRadius:"50%",background:"#1a1a2e",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:700,fontSize:13 }}>{initials(target.name)}</div>
          <div style={{ flex:1 }}>
            <p style={{ fontWeight:600,fontSize:15,color:"#1a202c",margin:0 }}>{target.name}</p>
            <p style={{ fontSize:12,color:"#718096",margin:0 }}>{target.industry}</p>
          </div>
        ...
        <div style={{ flex:1,overflowY:"auto",padding:"14px 16px" }}>
          {loading ? <p style={{ textAlign:"center",color:"#a0aec0",fontSize:13 }}>Loading...</p>
           : messages.length===0 ? <p style={{ textAlign:"center",color:"#a0aec0",fontSize:13,marginTop:60 }}>Start a conversation</p>
                ...
                {msgs.map(m=>{
                  ...
                  if (m.is_system) return (
                    <div key={m.id} style={{ textAlign:"center",margin:"8px 0" }}>
                      <div style={{ background:"#f0fdf4",border:"0.5px solid #bbf7d0",borderRadius:10,padding:"8px 14px",display:"inline-block",maxWidth:"90%",textAlign:"left" }}>
                        <p style={{ fontSize:13,color:"#15803d",margin:0,lineHeight:1.6,whiteSpace:"pre-line" }}>{m.content}</p>
                      </div>
                    </div>
                  );
                  return (
        ...
        <div style={{ padding:"12px 16px",borderTop:"0.5px solid #f1f5f9",display:"flex",gap:8 }}>
          ...
          <button onClick={send} disabled={!input.trim()||sending}
            style={{ padding:"9px 16px",background:input.trim()?"#1a1a2e":"#e2e8f0",color:input.trim()?"#fff":"#a0aec0",border:"none",borderRadius:10,fontSize:14,cursor:input.trim()?"pointer":"not-allowed",fontWeight:500 }}>
            Send
          </button>
        </div>
      </div>
